...
from hovsg.dataloader.goat import GoatDataset
...
class Graph:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        elif self.cfg.main.dataset == "goat":
            self.dataset = GoatDataset(dataset_cfg)
        else:
    ...
    def create_feature_map(self, save_path=None):
        ...
        for i in tqdm(range(0, len(self.dataset), self.cfg.pipeline.skip_frames), desc="Extracting features"):
            ...
            if rgb_image.size != depth_size:
                rgb_image = rgb_image.resize(depth_size)
    ...
    def save_masked_pcds(self, path, state="both"):
        ...
        to_remove = []
        for i, pcd in enumerate(self.mask_pcds):
            if len(pcd.points) < 50 or pcd.is_empty():
                to_remove.append(i)
