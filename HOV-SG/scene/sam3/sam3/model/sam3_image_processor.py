...
class Sam3Processor:
    ...
    def __init__(self, model, resolution=1008, device="cuda", confidence_threshold=0.5, use_presence_score=True):
        self.model = model
    ...
    @torch.inference_mode()
    def set_confidence_threshold(self, threshold: float, use_presence_score: bool = None, state=None):
    ...
    def _forward_grounding(self, state: Dict):
        ...
        # Debug prints
        # print(f"[DEBUG] Presence score: {presence_score.mean().item():.4f} (shape: {presence_score.shape})")
        # print(f"[DEBUG] Max raw object prob: {out_probs.max().item():.4f}")
