...
class MapObjectList(DetectionList):
    ...
    def to_serializable(self):
        ...
        for obj in self:
            ...
            # Handle clip_ft which can be a list of tensors or a single tensor
            if isinstance(s_obj_dict['clip_ft'], list):
                 s_obj_dict['clip_ft'] = [to_numpy(ft) for ft in s_obj_dict['clip_ft']]
            elif isinstance(s_obj_dict['clip_ft'], torch.Tensor):
                 s_obj_dict['clip_ft'] = to_numpy(s_obj_dict['clip_ft'])

            if isinstance(s_obj_dict.get('text_ft'), torch.Tensor):
                s_obj_dict['text_ft'] = to_numpy(s_obj_dict['text_ft'])
            
            if 'pcd' in s_obj_dict:
                s_obj_dict['pcd_np'] = np.asarray(s_obj_dict['pcd'].points)
                if hasattr(s_obj_dict['pcd'], 'colors'):
                     s_obj_dict['pcd_color_np'] = np.asarray(s_obj_dict['pcd'].colors)
                del s_obj_dict['pcd']

            if 'bbox' in s_obj_dict:
                 # OrientedBoundingBox
                 if hasattr(s_obj_dict['bbox'], 'get_box_points'):
                    s_obj_dict['bbox_np'] = np.asarray(s_obj_dict['bbox'].get_box_points())
                    s_obj_dict['bbox_color'] = np.asarray(s_obj_dict['bbox'].color)
                 del s_obj_dict['bbox']
    ...
    def load_serializable(self, s_obj_list):
        ...
        for s_obj_dict in s_obj_list:
            try:
                ...
                # Restore tensors
                if isinstance(new_obj['clip_ft'], list):
                    new_obj['clip_ft'] = [to_tensor(ft) for ft in new_obj['clip_ft']]
                else:
                ...
                if 'pcd_np' in new_obj:
                    ...
                    del new_obj['pcd_np']
                    if 'pcd_color_np' in new_obj: del new_obj['pcd_color_np']
                ...
                if 'bbox_np' in new_obj:
                    ...
                    if 'bbox_color' in new_obj:
                        new_obj['bbox'].color = new_obj['bbox_color']
                        del new_obj['bbox_color']
                    elif 'pcd_color_np' in new_obj and len(new_obj['pcd_color_np']) > 0: # fallback
