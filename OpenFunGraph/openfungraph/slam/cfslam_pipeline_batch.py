...
def process_cfg(cfg: DictConfig):
    ...
    if "save_root" in cfg and cfg.save_root is not None:
         cfg.save_root = Path(cfg.save_root)
    else:
         cfg.save_root = cfg.dataset_root
...
def main(cfg : DictConfig):
    ...
    for idx in trange(len(dataset)):
        ...
        if not os.path.exists(detections_path):
            print(f"Warning: Detection file not found: {detections_path}, skipping frame.")
            continue
