...
def create_or_load_colors(cfg, filename="gsa_classes_tag2text"):
    ...
    # Check for save_root in cfg
    root_path = cfg['save_root'] if 'save_root' in cfg else cfg['dataset_root']
    
    # get the classes, should be saved when making the dataset
    if not cfg.part_reg:
        classes_fp = root_path / cfg['scene_id'] / f"{filename}.json"
    else:
        classes_fp = root_path / cfg['scene_id'] / 'part' / f"{filename}.json"
    classes  = None
    
    # Fallback to dataset_root if not found in save_root (e.g. if classes were generated during detection which saves to dataset_root usually)
    # Actually, generate_gsa_results.py saves to dataset_root. 
    # If we want to move everything to autodl-tmp, we should probably check both or copy.
    # Let's check dataset_root first because detection runs there by default unless modified.
    if not classes_fp.exists() and 'save_root' in cfg:
         if not cfg.part_reg:
    ...
    class_colors  = None
    if 'save_root' in cfg:
        root_path = cfg['save_root']
    else:
        root_path = cfg['dataset_root']

    if not cfg.part_reg:
        class_colors_fp = root_path / cfg['scene_id'] / f"{filename}_colors.json"
    else:
        class_colors_fp = root_path / cfg['scene_id'] / 'part' / f"{filename}_colors.json"
    ...
    if class_colors_fp.exists():
        ...
        class_colors_fp.parent.mkdir(parents=True, exist_ok=True)
...
def create_object_pcd(depth_array, mask, cam_K, image, obj_color=None) -> o3d.geometry.PointCloud:
    ...
    # Filter depth consistent with HOV-SG/scene/model.py
    mask = np.logical_and(mask, (depth_array > 0.1) & (depth_array < 20.0))
    ...
    # Convert from OpenCV (Y-Down, Z-Forward) to OpenGL (Y-Up, -Z-Forward)
    # This aligns with the pose transformation T_gl_cv = diag(1, -1, -1, 1) applied in dataset loader
    y = -y
    z = -z
    ...
    # Open3D expects (N, 3) float64 arrays
    points = np.ascontiguousarray(points).astype(np.float64)
    
    # Ensure colors is (N, 3)
    if colors.ndim == 1 and colors.shape[0] == points.shape[0] * 3:
    ...
    if points.shape[0] != colors.shape[0]:
        print(f"Shape mismatch: points {points.shape}, colors {colors.shape}")
    ...
    try:
        ...
        print(f"Open3D Vector3dVector error: {e}")
        print(f"Points shape: {points.shape}, dtype: {points.dtype}")
        print(f"Colors shape: {colors.shape}, dtype: {colors.dtype}")
        print(f"Points contiguous: {points.flags['C_CONTIGUOUS']}")
        print(f"Colors contiguous: {colors.flags['C_CONTIGUOUS']}")
        raise e
...
def filter_gobs(
    ...
    for mask_idx in range(len(gobs['xyxy'])):
        ...
        if NOT_INTERE_CLASSES is not None:
            if class_name in NOT_INTERE_CLASSES:
                continue
...
def gobs_to_detection_list(
    ...
    for mask_idx in range(n_masks):
        ...
        try:
            ...
            # Class not in the pre-loaded list. 
            # This happens if we manually created the list and missed some.
            # We can either skip or add it. Adding is safer for logic but we need to ensure consistency.
            # For now, let's treat it as -1 or append?
            # Append might break color mapping if not updated.
            # Let's just print warn and set to -1 (unknown) or skip?
            # Fusion logic might depend on ID.
            # Let's append to class_names if it's a list.
            if isinstance(class_names, list):
                class_names.append(class_name)
                global_class_id = len(class_names) - 1
            else:
                global_class_id = -1
        ...
        detected_object = {
            ...
            # 'local_pcd': [np.array(camera_object_pcd.points)],
