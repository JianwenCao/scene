...
class GoatDataset(GradSLAMDataset):
    def __init__(
        self,
        config_dict,
        basedir,
        ...
        **kwargs,
        ...
        # Load intrinsics from sparse/0/cameras.txt or fallback to config
        cameras_path = os.path.join(self.input_folder, "sparse", "0", "cameras.txt")
        if os.path.exists(cameras_path):
             with open(cameras_path, "r") as f:
                ...
                if len(line) >= 8:
                    config_dict["camera_params"]["fx"] = float(line[4])
                    config_dict["camera_params"]["fy"] = float(line[5])
                    config_dict["camera_params"]["cx"] = float(line[6])
                    config_dict["camera_params"]["cy"] = float(line[7])
                    config_dict["camera_params"]["image_height"] = int(line[3])
                    config_dict["camera_params"]["image_width"] = int(line[2])
    ...
    def get_filepaths(self):
        ...
        color_paths = natsorted(glob.glob(os.path.join(image_dir, "*.png")))
        depth_paths = natsorted(glob.glob(os.path.join(depth_dir, "*.npy"))) 
    ...
    def load_poses(self):
        pose_path = os.path.join(self.input_folder, "local_pos.txt")
        poses = []
        poses_dict = {}
        ...
        # We need to re-fetch filepaths because super().__init__ already fetched them but we need to filter them
        # Wait, super().__init__ calls get_filepaths() then load_poses().
        # load_poses should update self.color_paths etc. if filtering happens.
        
        # We assume self.color_paths matches get_filepaths() result (which it does)
        # We need to filter based on available poses.
        
        new_poses = []
        new_color_paths, new_depth_paths, new_embedding_paths = [], [], []
        
        for i, color_p in enumerate(self.color_paths):
            try:
                basename = os.path.basename(color_p)
                # handle imgXXXX.png
                idx_str = basename.replace("img", "").split(".")[0]
                idx = int(idx_str)
                if idx in poses_dict:
                    new_poses.append(torch.from_numpy(poses_dict[idx]).float())
                    new_color_paths.append(color_p)
                    new_depth_paths.append(self.depth_paths[i])
                    if self.embedding_paths:
                        new_embedding_paths.append(self.embedding_paths[i])
            except:
                continue
        ...
        if self.embedding_paths:
            self.embedding_paths = new_embedding_paths
...
def get_dataset(dataconfig, basedir, sequence, **kwargs):
    ...
    elif config_dict['dataset_name'].lower() in ['goat']:
        return GoatDataset(config_dict, basedir, sequence, **kwargs)
    else:
